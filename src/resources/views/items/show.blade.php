            ...
            <div class="item-detail__grid">
                <div class="item-detail__media">
                    <div class="item-detail__image {{ $item->is_sold ? 'is-sold' : '' }}">
                        <img class="item-detail__img" src="{{ asset('storage/' . $item->image_path) }}"
                            alt="{{ $item->name }}">
                    ...
                    <div class="item-detail__meta">
                        <div class="meta-icon">
                            <button class="like-btn" type="button" aria-label="いいね"
                                data-auth="{{ auth()->check() ? 'true' : 'false' }}" data-login-url="{{ route('login') }}">

                                @if ($isLiked)
                                    <img class="like-btn__img" src="{{ asset('images/ハートロゴ_ピンク.png') }}"
                                        data-off="{{ asset('images/ハートロゴ_デフォルト.png') }}"
                                        data-on="{{ asset('images/ハートロゴ_ピンク.png') }}" data-liked="true" alt="いいね">
                                @else
                                    <img class="like-btn__img" src="{{ asset('images/ハートロゴ_デフォルト.png') }}"
                                        data-off="{{ asset('images/ハートロゴ_デフォルト.png') }}"
                                        data-on="{{ asset('images/ハートロゴ_ピンク.png') }}" data-liked="false" alt="いいね">
                                @endif
                            </button>
